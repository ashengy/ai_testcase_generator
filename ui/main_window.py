...
class DeepSeekTool(QMainWindow, Ui_DeepSeekTool):
    ...
    def init_ui(self):
        ...
        self.comboBox.setCurrentIndex(0)  # 行业默认选中"无"
...
"""
    ...
    def stop_generate(self):
        ...
        try:
            if hasattr(self, 'image_thread') and self.image_thread.isRunning():
                self.image_thread.terminate()  # 强制终止图片分析线程
    ...
    def start_image_analyzer(self):
        try:
            ...
            # 连接进度信号
            self.image_thread.progress.connect(self.on_image_progress)
    ...
    def on_image_progress(self, current, total, message):
        """图片分析进度更新"""
        self.update_talking(f"分析进度: {current}/{total} - {message}")
